...
describe Tennis do
    ...
    it 'return 40-0 when player one scores three points' do
    ...
    it 'return 0-40 when player two scores three points' do
        3.times { tennis.win_point(:player_two) }
        expect(tennis.score).to eq('0-40')
    end 

    it "return player one wins if player one scores four times without loss" do
        4.times { tennis.win_point(:player_one) }
        expect(tennis.score).to eq('Player one wins')
    end

    it "return player two wins if player two scores four times without loss" do
    ...
    context "when deuce" do
        ...
        it "return player one wins if player one scores after an advantage" do
            2.times { tennis.win_point(:player_one) }
            expect(tennis.score).to eq('Player one wins')
        end

        it "return player two wins if player two scores after an advantage" do
            2.times { tennis.win_point(:player_two) }
            expect(tennis.score).to eq('Player two wins')
        end
        ...
        it "returns player one wins if player one scores twice from a double duece" do
            3.times do
                tennis.win_point(:player_two)
                tennis.win_point(:player_one) 
            end
